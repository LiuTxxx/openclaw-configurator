...
- **启动**：双击下载的 `.exe` 文件，会弹出一个黑色命令行窗口，浏览器自动打开配置页面
- **⚠️ 关闭 ⚠️**：**关掉那个黑色命令行窗口**即可
...
- **关闭**：双击启动的关掉窗口即可；终端启动的按 **Ctrl+C**

> **⚠️ 注意：关掉浏览器标签页并不会停止程序！** 程序运行在终端/命令行窗口里，必须关掉那个窗口或按 Ctrl+C 才会停止。本工具不是后台服务，用完即关。
